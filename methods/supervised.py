...
def fine_tune_model(
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            print(f"Epoch {epoch + 1}/{epochs} - Loss: {avg_loss}, "
                  f"Train F1: {f1_train}, Precision: {precision_train}, Recall: {recall_train}, "
                  f"Test F1: {f1_test}, Precision: {precision_test}, Recall: {recall_test}")
            ...
            if f1_test > best_f1:
                ...
                if save_path:  # 检查是否指定了保存路径
                    ...
                    tokenizer.save_pretrained(save_path)
                    print(f"New best model saved with F1 score {best_f1} at epoch {epoch + 1}")
    ...
    plt.plot(range(1, epochs + 1), train_precision_list, label='Train Precision')
    plt.plot(range(1, epochs + 1), test_precision_list, label='Test Precision')
    plt.plot(range(1, epochs + 1), train_recall_list, label='Train Recall')
    plt.plot(range(1, epochs + 1), test_recall_list, label='Test Recall')
    plt.xlabel('Epoch')
    plt.ylabel('Score')
